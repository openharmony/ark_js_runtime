...
{
    auto *tempString = const_cast<NativeAreaAllocator *>(vm_->GetNativeAreaAllocator())->New<CString>(std::move(as));
    CString *oldString = FindOrInsertString(tempString);
    if (tempString != oldString) {
        const_cast<NativeAreaAllocator *>(vm_->GetNativeAreaAllocator())->Delete(tempString);
        return oldString;
    }
    return tempString;
}
...
void StringHashMap::Clear()
{
    for (auto it : hashmap_) {
        if (it.second != nullptr) {
            const_cast<NativeAreaAllocator *>(vm_->GetNativeAreaAllocator())->Delete(it.second);
        }
    }
}
}  // namespace panda::ecmascript
