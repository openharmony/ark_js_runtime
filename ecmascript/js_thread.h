...
public:
    ...
    using GCStateBits = BitField<GCState, 0, CONCURRENT_MARKING_BITFIELD_NUM>;
    ...
    void SetGCState(GCState status)
    {
        uint64_t newVal = GCStateBits::Update(threadStateBitField_, status);
        threadStateBitField_ = newVal;
    }

    bool IsNotBeginMark() const
    {
        auto status = GCStateBits::Decode(threadStateBitField_);
        return status == GCState::NOT_BEGIN_MARK;
    }

    bool IsMarking() const
    {
        auto status = GCStateBits::Decode(threadStateBitField_);
        return status == GCState::MARKING;
    }

    bool IsMarkFinished() const
    {
        auto status = GCStateBits::Decode(threadStateBitField_);
        return status == GCState::MARK_FINISHED;
    }
