...
GateRef Stub::FindOwnElement2(GateRef glue, GateRef elements, GateRef index, GateRef isDict,
                              GateRef attr, GateRef indexOrEntry)
{
    ...
    Label end(env);
    Branch(Word32Equal(ZExtInt1ToInt32(isDict), GetInt32Constant(0)), &notDictionary, &isDictionary);
    Bind(&notDictionary);
    {
        GateRef elementsLength =
            Load(MachineType::UINT32, elements, GetArchRelateConstant(TaggedArray::GetLengthOffset()));
        ...
        Bind(&notOutOfElements);
        {
            ...
            Bind(&notHole);
            {
                Store(MachineType::UINT32, glue, attr, GetArchRelateConstant(0),
                      GetInt32Constant(PropertyAttributes::GetDefaultAttributes()));
                Store(MachineType::UINT32, glue, indexOrEntry, GetArchRelateConstant(0), index);
                result = value;
                Jump(&exit);
            }
        }
    ...
    Bind(&isDictionary);
    {
        ...
        {
            Store(MachineType::UINT32, glue, attr, GetArchRelateConstant(0),
                  GetAttributesFromDictionary<NumberDictionary>(elements, entry));
            Store(MachineType::UINT32, glue, indexOrEntry, GetArchRelateConstant(0), entry);
            result = GetValueFromDictionary<NumberDictionary>(MachineType::TAGGED, elements, entry);
            Jump(&exit);
        }
        Bind(&negtiveOne);
        Jump(&exit);
