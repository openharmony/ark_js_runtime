...
HWTEST_F_L0(BuiltinsMapTest, DeleteAndRemove)
{
    ...
    for (int i = 0; i < 40; i++) {
        keyArray[3] = '1' + i;
        JSHandle<JSTaggedValue> key(thread, factory->NewFromString(keyArray).GetTaggedValue());
        auto ecmaRuntimeCallInfo = TestHelper::CreateEcmaRuntimeCallInfo(thread, JSTaggedValue::Undefined(), 8);
        ecmaRuntimeCallInfo->SetFunction(JSTaggedValue::Undefined());
        ecmaRuntimeCallInfo->SetThis(map.GetTaggedValue());
        ecmaRuntimeCallInfo->SetCallArg(0, key.GetTaggedValue());
...
HWTEST_F_L0(BuiltinsMapTest, Species)
{
    ...
    JSHandle<EcmaString> str = factory->NewFromString("Map");
    ...
    JSHandle<JSTaggedValue> key1(factory->NewFromString("set"));
    ...
    JSHandle<JSTaggedValue> key7(factory->NewFromString("get"));
