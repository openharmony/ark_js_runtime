    ...
    std::vector<std::tuple<uint8_t *, SplitPoint, std::vector<uint8_t *>>> &bytecodeBlockInfo)
{
    ...
    cda.EnumerateTryBlocks([method, &byteCodeCurPrePc, &bytecodeBlockInfo, &byteCodeException](
        ...
        if (!flag) {
            // pre block
            bytecodeBlockInfo.emplace_back(byteCodeCurPrePc[tryStartPc], SplitPoint::END, std::vector<uint8_t *>(1, tryStartPc));
        }
        bytecodeBlockInfo.emplace_back(tryStartPc, SplitPoint::START, std::vector<uint8_t *>(1, tryStartPc)); // try block
        flag = false;
        for (size_t i = 0; i < bytecodeBlockInfo.size(); i++) {
            if (std::get<1>(bytecodeBlockInfo[i]) == SplitPoint::START) {
                continue;
        ...
        bytecodeBlockInfo.emplace_back(tryEndPc, SplitPoint::START, std::vector<uint8_t *>(1, tryEndPc));  // next block
        return true;
    });
    ...
    std::vector<std::tuple<uint8_t *, SplitPoint, std::vector<uint8_t *>>> &bytecodeBlockInfo)
{
    ...
    for(auto iter = endBlockPc.begin(); iter != endBlockPc.end(); iter+=2) {
        bytecodeBlockInfo.emplace_back(std::make_tuple(*iter, SplitPoint::END, std::vector<uint8_t *>(1, *(iter+1))));
    }
