...
#define UPDATE_HOTNESS(_sp)                                                               \
    ...
    {                                                                                     \
        ...
        varProfileTypeInfo = CallRuntime(subDescriptor, glue,                             \
            GetWord64Constant(FAST_STUB_ID(UpdateHotnessCounter)), { glue, (_sp) });      \
...
DECLARE_ASM_HANDLER(HandleStOwnByNamePrefId32V8)
{
    ...
    Bind(&slowPath);
    {
        ...
        result = CallRuntime(StOwnByName, glue, GetWord64Constant(FAST_STUB_ID(StOwnByName)),
                             { glue, receiver, propKey, acc });
        Jump(&checkResult);
    }
    Bind(&checkResult);
...
DECLARE_ASM_HANDLER(HandleJnezImm8)
{
    ...
    Label accIsDouble(env);
    Label last(env);
    Branch(Word64Equal(ChangeTaggedPointerToInt64(acc), TaggedTrue()), &accEqualTrue, &accNotEqualTrue);
    Bind(&accNotEqualTrue);
    {
        Branch(TaggedIsInt(acc), &accIsInt, &accNotInt);
        Bind(&accIsInt);
        {
            Branch(Word32Equal(TaggedGetInt(acc), GetInt32Constant(0)), &accNotInt, &accEqualTrue);
        }
        Bind(&accNotInt);
        {
            Branch(TaggedIsDouble(acc), &accIsDouble, &last);
...
DECLARE_ASM_HANDLER(HandleReturnDyn)
{
    ...
    Bind(&updateHotness);
    {
        ...
        Store(MachineType::INT32, glue, method,
              GetArchRelateConstant(JSMethod::HOTNESS_COUNTER_OFFSET), *varHotnessCounter);
        Jump(&tryContinue);
    }

    Bind(&tryContinue);
    varSp = Load(MachineType::NATIVE_POINTER, frame,
...
DECLARE_ASM_HANDLER(HandleSuspendGeneratorPrefV8V8)
{
    ...
    GateRef res = CallRuntime(suspendGenerator, glue, GetWord64Constant(FAST_STUB_ID(SuspendGenerator)),
        { glue, genObj, value });
...
DECLARE_ASM_HANDLER(HandleAdd2DynPrefV8)
{
        ...
        Bind(&rightIsNumber);
        {
            ...
            Bind(&leftIsInt);
            {
                ...
                Bind(&rightIsInt);
                {
                    ...
                    Branch(Word32Equal(*opNumber0, GetInt32Constant(0)), &leftIsZero, &leftNotZero);
                    Bind(&leftIsZero);
                    {
                    ...
                    Bind(&leftNotZero);
                    {
                        ...
                        Branch(Word32Equal(*opNumber1, GetInt32Constant(0)), &rightIsZero, &rightNotZero);
                        Bind(&rightIsZero);
                        {
                            GateRef res = Int32Add(*opNumber0, *opNumber1);
                            varAcc = IntBuildTaggedWithNoGC(res);
                        ...
                        Bind(&rightNotZero);
                        {
                            ...
                            Bind(&leftIsPositive);
                            {
                                ...
                                Branch(Int32GreaterThan(*opNumber1, Int32Sub(GetInt32Constant(INT32_MAX), *opNumber0)), &positiveOverflow, &notPositiveOverflow);
                            ...
                            Bind(&leftIsNegative);
                            {
                                ...
                                Branch(Int32LessThan(*opNumber1, Int32Sub(GetInt32Constant(INT32_MIN), *opNumber0)), &negativeOverflow, &notNegativeOverflow);
    ...
    Bind(&leftNotNumberOrRightNotNumber);
    {
        ...
        GateRef taggedNumber = CallRuntime(add2Dyn, glue, GetWord64Constant(FAST_STUB_ID(Add2Dyn)),
                                           {glue, left, right});
...
DECLARE_ASM_HANDLER(HandleSub2DynPrefV8)
{
    ...
    Bind(&leftNotNumberOrRightNotNumber);
    {
        ...
        GateRef taggedNumber = CallRuntime(sub2Dyn, glue, GetWord64Constant(FAST_STUB_ID(Sub2Dyn)),
                                           {glue, left, right});
