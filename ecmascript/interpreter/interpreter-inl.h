...
JSTaggedValue EcmaInterpreter::Execute(JSThread *thread, const CallParams& params)
{
    ...
    if (callType == NORMAL_CALL_TYPE) {  // fast path
        ...
        size_t copyArgs = std::min(numDeclaredArgs, params.argc);
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
        ...
        if (haveExtra) {
            copyArgs += params.argc;
            frameSize += std::max(numDeclaredArgs, copyArgs) + 1;  // 1: to store numActualArgs
        } else {
            copyArgs = std::min(numDeclaredArgs, params.argc + copyArgs);
        ...
        if (haveFunc) {
            // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
            newSp[startIdx++] = reinterpret_cast<JSTaggedType>(params.callTarget);
        }
        if (haveNewTarget) {
            // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
            newSp[startIdx++] = params.newTarget;
        }
        if (haveThis) {
            // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
            newSp[startIdx++] = params.thisArg;
        }
        for (size_t i = 0; startIdx < numVregs + copyArgs; i++) {
            // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
            newSp[startIdx++] = params.argv[i];
        }
    ...
    JSTaggedValue ecmaModule = thisFunc->GetEcmaModule();
    thread->GetEcmaVM()->GetEcmaModuleManager()->AddSourceModule(ecmaModule);
#if ECMASCRIPT_ENABLE_ACTIVE_CPUPROFILER
    CpuProfiler::IsNeedAndGetStack(thread);
#endif
    thread->CheckSafepoint();
    ...
    thread->GetEcmaVM()->GetNotificationManager()->MethodExitEvent(thread, method);
    thread->GetEcmaVM()->GetEcmaModuleManager()->PopSourceModule();
...
JSTaggedValue EcmaInterpreter::GeneratorReEnterInterpreter(JSThread *thread, JSHandle<GeneratorContext> context)
{
    ...
    thread->SetCurrentSPFrame(newSp);
    thread->GetEcmaVM()->GetNotificationManager()->MethodEntryEvent(thread, method);
    JSTaggedValue ecmaModule = context->GetEcmaModule();
    thread->GetEcmaVM()->GetEcmaModuleManager()->AddSourceModule(ecmaModule);
...
#include "templates/debugger_instruction_dispatch.inl"
    ...
    }
    {
        ...
        handlerCall:
            ...
            if (method->IsNative()) {
                ...
                if (callRange) {
                    size_t copyArgs = actualNumArgs + NUM_MANDATORY_JSFUNC_ARGS - 2;  // 2: skip func and new target
                    if (!callThis) {
                        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
                        newSp[startIdx++] = JSTaggedValue::VALUE_UNDEFINED;
                        // skip this
                ...
                if (callType == NORMAL_CALL_TYPE) {  // fast path
                    ...
                    if (haveExtra) {
                        ...
                        frameSize += std::max(numDeclaredArgs, copyArgs) + 1;  // 1: to store actualNumArgs
                    } else {
                ...
                JSTaggedValue ecmaModule = JSFunction::Cast(thisFunc)->GetEcmaModule();
                thread->GetEcmaVM()->GetEcmaModuleManager()->AddSourceModule(ecmaModule);
                DISPATCH_OFFSET(0);
            }
        }
    ...
    HANDLE_OPCODE(HANDLE_DEFINEFUNCDYN_PREF_ID16_IMM16_V8) {
        ...
        result->SetEcmaModule(thread, thread->GetEcmaVM()->GetEcmaModuleManager()->GetCurrentSourceModule());
    ...
    HANDLE_OPCODE(HANDLE_LDMODULEVAR_PREF_ID32_IMM8) {
        ...
        JSTaggedValue moduleVar = SlowRuntimeStub::LdModvarByName(thread, itemName, flag != 0);
    ...
    HANDLE_OPCODE(HANDLE_DEFINECLASSWITHBUFFER_PREF_ID16_IMM16_IMM16_V8_V8) {
        ...
        cls->SetEcmaModule(thread, thread->GetEcmaVM()->GetEcmaModuleManager()->GetCurrentSourceModule());
...
void EcmaInterpreter::InterpreterFrameCopyArgs(JSTaggedType *newSp, uint32_t numVregs, uint32_t numActualArgs,
                                               uint32_t numDeclaredArgs, bool haveExtraArgs)
{
    size_t i = 0;
    for (; i < numVregs; i++) {
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
